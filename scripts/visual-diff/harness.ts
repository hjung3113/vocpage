...
/** Register process cleanup on exit signals. */
...
export async function bootHarness(opts: HarnessOptions = {}): Promise<HarnessHandles> {
  ...
  spawnProcess('npx', ['http-server', protoDir, '-p', String(protoPort), '--cors', '-c-1'], {
    label: 'proto-server',
  });

  // --- Spawn Vite dev server ---
  spawnProcess('npm', ['run', '-w', 'frontend', 'dev'], {
    label: 'vite-dev',
  ...
  // Intercept all API calls — use a broad pattern then filter.
  // /api/auth/me → { user } shape (checked by getMe())
  // /api/auth/mock-login → { user } shape (checked by mockLogin())
  await reactContext.route('**/*', async (route) => {
