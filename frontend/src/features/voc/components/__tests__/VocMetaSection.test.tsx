...
describe('VocMetaSection', () => {
  it('기본 렌더링 — 8개 메타 필드(created_at/status/priority/type/assignee/due_date/system/menu) 모두 표시', () => {
    render(<VocMetaSection voc={baseVoc} assigneeMap={assigneeMap} vocTypeMap={vocTypeMap} />);
  ...
  it('등록일 필드가 created_at 날짜(YYYY-MM-DD)로 표시됨', () => {
    render(<VocMetaSection voc={baseVoc} assigneeMap={assigneeMap} />);
  ...
  it('due_date === null → "—" 표시', () => {
    ...
    render(<VocMetaSection voc={voc} assigneeMap={assigneeMap} />);
