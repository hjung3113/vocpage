...
describe('NoticePage — list (user mode)', () => {
  it('숨김·만료·삭제된 공지는 노출되지 않는다', () => {
    roleMock.mockReturnValue({
      role: 'user', isUser: true, isDev: false, isManager: false, isAdmin: false,
    });
    const visible = { ...baseNotice, id: 'a', title: '노출됨' };
    const hidden = { ...baseNotice, id: 'b', title: '숨김', is_visible: false };
    const expired = { ...baseNotice, id: 'c', title: '만료', visible_to: '2020-01-01T00:00:00.000Z' };
    const deleted = { ...baseNotice, id: 'd', title: '삭제', deleted_at: '2026-04-01T00:00:00.000Z' };
  ...
  it('중요도 배지가 토큰 색상으로 매핑된다', () => {
    ...
    renderPage();
    expect(screen.getByTestId('notice-level-badge-normal')).toHaveStyle('background: var(--status-blue)');
    expect(screen.getByTestId('notice-level-badge-important')).toHaveStyle('background: var(--status-amber)');
    expect(screen.getByTestId('notice-level-badge-urgent')).toHaveStyle('background: var(--status-red)');
  });
...
describe('NoticePage — 관리 버튼 가시성 (§10.5.1)', () => {
  ...
  it('dev 에게는 DOM 자체가 없다', () => {
    roleMock.mockReturnValue({
      role: 'dev', isUser: false, isDev: true, isManager: false, isAdmin: false,
    });
    renderPage();
    expect(screen.queryByTestId('notice-admin-toggle')).not.toBeInTheDocument();
  });
