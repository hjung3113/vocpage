...
- 전체 화면 상태는 URL 파라미터(`?view=full`) 또는 sessionStorage로 유지 (새로고침 시 복원 선택적).
...
- **상세 명세**: `.omc/plans/dashboard-feature.md` 참조
...
| 분포 탭 (도넛) | 상태 / 우선순위 / 유형별 건수·비율 |
| 우선순위×상태 매트릭스 | 4×5 히트맵, 셀 클릭 → VOC 리스트 필터 이동 |
| 드릴다운 히트맵 | 전체 → 시스템별 → 메뉴별 계층 탐색 |
| Created vs. Resolved | 최근 12주 주간 라인 차트 (날짜 필터 무관, 고정) |
| 태그별 분포 | Top 10 태그 가로 바 차트 |
| 시스템별 현황 카드 | 드릴다운 (최대 8개 + 더보기) |
...
| `recharts` | LineChart / BarChart (lazy import) |
