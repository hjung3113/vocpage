...
(function () {
  ...
  function nextFaqId() {
    var ids = (window.FAQS || []).map(function (f) { return f.id; });
    return ids.length ? Math.max.apply(null, ids) + 1 : 1;
  }

  function nextCatId() {
    var ids = (window.FAQ_CATEGORIES || []).map(function (c) { return c.id; });
    return ids.length ? Math.max.apply(null, ids) + 1 : 1;
  }

  function categoryOptions(selected) {
    return (window.FAQ_CATEGORIES || []).map(function (c) {
      return '<option value="' + esc(c.name) + '"' + (c.name === selected ? ' selected' : '') + '>' + esc(c.name) + '</option>';
    }).join('');
  }

  // ── FAQ Modal
  function getFaqModal() { return document.getElementById('nfaFaqModal'); }
  ...
  function saveFaqModal() {
    ...
    if (!a) { toast('답변을 입력하세요.', 'warn'); return; }
    if (faqModalMode === 'create') {
      window.FAQS = window.FAQS || [];
      window.FAQS.unshift({ id: nextFaqId(), category: category, q: q, a: '<p>' + esc(a) + '</p>', visible: visible });
      toast('FAQ가 등록되었습니다.', 'ok');
    } else {
      var f = (window.FAQS || []).find(function (x) { return x.id === faqEditingId; });
      if (f) { f.q = q; f.a = '<p>' + esc(a) + '</p>'; f.category = category; f.visible = visible; }
  ...
  // ── Category Modal
  function getCatModal() { return document.getElementById('nfaCatModal'); }
  ...
  function openEditCategoryModal(id) {
    var c = (window.FAQ_CATEGORIES || []).find(function (x) { return x.id === id; });
    if (!c) return;
    catModalMode = 'edit';
    catEditingId = id;
    renderCatModal(c);
  ...
  function renderCatModal(data) {
    var bg = getCatModal();
    if (!bg) return;
    bg.querySelector('.nfa-modal-title').textContent = catModalMode === 'create' ? '카테고리 추가' : '카테고리 편집';
  ...
  function saveCatModal() {
    ...
    if (!name) { toast('카테고리 이름을 입력하세요.', 'warn'); return; }
    if (catModalMode === 'create') {
      ...
      var c = (window.FAQ_CATEGORIES || []).find(function (x) { return x.id === catEditingId; });
      if (c) { c.name = name; c.order = order; c.visible = visible; }
  ...
  function buildModalDoms() {
    if (!document.getElementById('nfaFaqModal')) {
      ...
      faqBg.addEventListener('click', function (e) { if (e.target === faqBg) closeFaqModal(); });
    ...
    if (!document.getElementById('nfaCatModal')) {
      ...
      catBg.addEventListener('click', function (e) { if (e.target === catBg) closeCatModal(); });
