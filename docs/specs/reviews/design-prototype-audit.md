...
> **목적**: `prototype/prototype.html`(4090 lines)과 `docs/specs/requires/uidesign.md`(742 lines + §11) 사이의 누락·불일치를 색출하고, 향후 React 구현 시 `tokens.ts` → CSS 변수 경유의 단일 출처를 보장한다.
>
...
| uidesign.md `:root` 외 본문에 출현하는 raw color | 다수 (§3 색상 팔레트는 정의 표라 합법, §5 컴포넌트 본문에도 일부) |
| uidesign.md에 누락된 컴포넌트 spec               | **12건** (Admin·Notice·FAQ 영역)                                  |

> ⚠️ "108건" 중 다수는 `:root` 토큰 **정의** 라인(§prototype.html L13~50)이며 합법. 본 audit는 `:root` 블록 외부의 raw color를 위반으로 본다.

---

## 2. 컴포넌트 갭 — uidesign.md 신규 §13에 편입 필요

| 컴포넌트 클래스 (prototype)                                   | 위치 (line)                | uidesign.md 현황                   | 처리                                                         |
| ------------------------------------------------------------- | -------------------------- | ---------------------------------- | ------------------------------------------------------------ |
...
## 3. 색상 토큰 갭 — uidesign.md §10 보강 필요
...
| `prototype.html` | 399~433 | 상태 배지 (`s-접수`/`s-검토중`/`s-처리중`/`s-완료`) inline `light-dark(oklch())` | uidesign.md §5 Status Badge에 5종 토큰 그룹 (`--status-{name}-{bg                                        | fg  | border | dot}`) 신규 → prototype 리팩터 |
| `uidesign.md`    | 151     | `color: #ffffff;` 본문                                                           | §3 'Don't use pure `#000000`/`#ffffff`' 자체 위반 → `oklch(100% 0 0)` 또는 `var(--text-on-brand)`로 정정 |
| `uidesign.md`    | 239~242 | 표 본문에 raw oklch (정의 표)                                                    | 합법 (color reference 표)                                                                                |
| `uidesign.md`    | 262~263 | Priority Badge 색상 표                                                           | 합법 (정의)                                                                                              |
| `uidesign.md`    | 303     | sidebar count badge 'color: #fff' 본문                                           | 위 §13.10에서 토큰화                                                                                     |

> **결정**: `--text-on-brand` 신규 토큰을 §10에 추가 (`light-dark(oklch(100% 0 0), oklch(100% 0 0))` — 의미 = "브랜드/색상 배경 위 텍스트"). 모든 `#fff` 본문 사용처를 이것으로 치환 권고.

## 4. uidesign.md 자체 룰 위반
...
1. uidesign.md에 **§13 'Admin · Notice · FAQ Components'** 신규 — Admin·공지·FAQ·관리 진입 버튼·로그인 팝업·사이드바 카운트 배지 12건 spec 정식화. 영문 작성.
...
- ✅ uidesign.md §13 추가로 React 컴포넌트 작성 시 spec-first 보장.
...
5. uidesign.md §5 Status Badge 토큰화 별도 ticket — prototype L399~433 위반 정리.

---

## 6. 후속 티켓 (next-session-tasks.md에 등록)

- [ ] T1: uidesign.md §13 작성 (영문 700~1000 lines 추가)
- [ ] T2: uidesign.md §10에 `--role-dev-{bg|fg|border}` + `--text-on-brand` 토큰 추가
- [ ] T3: uidesign.md §8 Don't에 raw color literal 금지 항목 추가
- [ ] T4: uidesign.md §5 Status Badge 토큰 그룹 정식화 (prototype L399~433 정리 ticket 별도)
