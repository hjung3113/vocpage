...
| 외부 마스터    | admin manager      | external-masters.md L118 "Manager/Admin 화면에 콜드 스타트 모드 배지 노출" 직접 인용 |
...
R1 → R2 변경: 결과 검토 nav `+dev` (architect P1, §8.2.1 line 70 근거).
R2 → R3 변경: 외부 마스터 `admin manager` 복원 (verifier P1, external-masters.md L118 "Manager/Admin 화면" 명시 인용 — R2의 admin only 강등은 spec drift였음).
