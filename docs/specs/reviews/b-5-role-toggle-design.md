...
| Nav 항목       | data-role-allow    | 근거 (spec 직접 인용)                                                              |
| -------------- | ------------------ | ---------------------------------------------------------------------------------- |
| 담당 VOC       | admin manager dev  | §8.3 "전체 VOC 조회" Manager/Admin=✅, Dev=✅, User=❌                             |
| 대시보드       | admin manager dev  | §8.3 "Dashboard 접근" Dev=✅, User=❌                                              |
| 태그 규칙 관리 | admin manager      | §8.3 "태그 규칙 관리(생성/수정/삭제)" Manager+Admin                                |
| 시스템/메뉴    | admin              | §8.3 "시스템/메뉴/유형 관리" Admin only                                            |
| 유형 관리      | admin              | 동상                                                                               |
| 사용자 관리    | admin              | §8.3 "사용자 role 관리" Admin only                                                 |
| 결과 검토      | admin manager dev  | §8.2.1 line 70 "전환 권한: Manager/Admin, Dev(본인 담당 VOC 한정)"                 |
| 태그 마스터    | admin manager      | §8.8.2 + §9.4.6 "신규 태그 생성은 Admin/Manager 전용 관리 페이지에서만"            |
| 휴지통         | admin              | §8.3 "VOC Soft Delete / 복원" Admin only                                           |
| 외부 마스터    | admin              | external-masters.md §5 `/api/admin/masters/refresh` admin route + system 설정 성격 |
| 공지/FAQ       | (gate 없음 — 모두) | 사이드바는 **읽기 진입점**. 작성/관리는 D19 별도 진입점(`?mode=admin`, B-4a/B-4b)  |
| 새 VOC 등록    | (gate 없음 — 모두) | §8.3 "VOC 작성" 4역할 모두 ✅                                                      |
...
| `prototype/js/role-state.js`  | NEW — 252줄 (≤300 룰 준수, R2 ARIA + 키보드 + 보안 주석 추가)          |
...
## 검증 (smoke, R2 — 5인 리뷰 fix 반영 후)

```text
admin   → 10 nav visible (전체 admin 메뉴 + 대시보드)
manager → 5 nav visible (admin-only 5건 차단: system-menu, voc-type, users, trash, external-masters)
dev     → 3 nav visible (담당 VOC + 대시보드 + 결과 검토[own VOC])
user    → 0 nav visible (대시보드 차단, admin section 전체 collapse)
```

R1 → R2 변경: 결과 검토 nav `+dev` (architect P1), 외부 마스터 `admin only` (architect P1, external-masters.md §5 근거).
