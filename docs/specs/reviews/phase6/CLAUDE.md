...
## Phase 7 진입 전 Preflight 리뷰 (2026-04-24)

적대적 리뷰(critic vs analyst) 결과 — **즉시 처리 항목 해소 후 Phase 7 착수**.

→ **`reviews/phase6/phase7-preflight-adversarial-review.md`** 참조

즉시 처리 필수:

1. `backend/migrations/003_vocs.sql` — `parent_id` / `issue_code` / `due_date` 컬럼 누락 확인
2. `vocs.status` enum `'접수'` vs `'접수됨'` 3-way 통일
3. `author_id` vs `reporter_id` 컬럼명 통일
4. openapi.yaml 누락 엔드포인트 추가 (Result Review, Payload, Timeline, 공지/FAQ)
5. 세션 스토어 결정 (`connect-pg-simple` 권장)
