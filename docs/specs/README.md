...
| #   | 책임                                  | 정본 파일                                                                                                                          |
| --- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Governance / 작업 룰 / 본 정책 (정본) | `CLAUDE.md` (root) + 8-keep sub-`CLAUDE.md` (`.claude`, `frontend`, `frontend/src`, `backend`, `backend/src`, `docs`, `prototype`) |
...
- **Folder `CLAUDE.md` 8-keep set** — root, `.claude`, `frontend`, `backend`, `frontend/src`, `backend/src`, `docs`, `prototype`. PR이 한 keep의 `## Role` / `## When to look where`를 바꿀 때만 갱신한다.
  - 8-keep set 외 신규 leaf `CLAUDE.md` 생성 금지 — 디렉토리 가이드는 가장 가까운 ancestor에 흡수한다.
  - delete 시 stub (`→ ancestor.md`) 금지 — auto-loaded noise 방지.
