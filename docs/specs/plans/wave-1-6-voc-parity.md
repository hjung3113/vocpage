...
> 인덱스 규칙: `docs/specs/README.md §7`. Wave 계보: `wave-index.md`. 머지된 leaf 상세: `progress-archive.md` Wave 1.6 절.
...
| **δ** | C-12 VocReviewDrawer → C-13 VocReviewSections                                                                                           | sequential | δ shell 의존. C-13은 **탭 폐기 + 평평 섹션 골격 추출** (rename: VocReviewTabs → VocReviewSections). 각 섹션 내부 구성은 ε/η batch에서 채움                                                                                                                                                                                                                                                          |
| **ε** | C-14 VocCommentList ∥ C-15 VocInternalNotes ∥ C-16 VocSubTaskList                                                                       | 3 슬롯     | δ 머지 후 진입. VocReviewSections 의 children/composition 슬롯에 끼워넣는다                                                                                                                                                                                                                                                                                                                         |
| **ζ** | C-17 SidebarUserSwitcher → C-18 Sidebar ∥ C-19 Topbar                                                                                   | 2 슬롯     | C-18은 C-17 popover 의존. C-19는 **β의 NotifButton 머지 후** 시작 (props 충돌 회피)                                                                                                                                                                                                                                                                                                                 |
| **η** | C-20 VocTagPills ∥ C-21 VocActivityTimeline ∥ C-22 VocAttachmentUploadZone ∥ C-23 VocErrorSimSelector                                   | 4 슬롯     | ε 종료 후 진입. C-13에서 자리만 잡힌 prototype 섹션들의 실내용 채움. 각 leaf 진입 시 prototype 재확인 + spec 확정 필요 (현재는 placeholder만)                                                                                                                                                                                                                                                       |

- **AppShell**은 token-align 분류이므로 Phase C rebuild 대상 아님. Phase D 종합 검증에서 일괄 점검.
- Critical path (sequential): C-2.5 → B-add-2 → C-2.6 → C-5 → C-7 → C-11 → C-12 → C-13 → C-19. ≈ 9 단계. [AMENDED 2026-05-02 PR C-5: B-add-1 제거.] **C-20~C-23 (η batch)는 ε 종료 후 4-슬롯 병렬이라 critical path 외부**.
- 총 PR 수: rebuild 19 PR + addendum 1 PR (B-add-2) + audit 1 PR (C-2.5) + η batch 4 PR (C-20~C-23) = **25 PR**. 추정 7~11 세션. [AMENDED 2026-05-04 C-13 재정의: VocReviewTabs → VocReviewSections, prototype 섹션 4종 신규 leaf 등록 (η batch).]
...
## 12. Post-merge 사용자 피드백 — archived

C-9 직후 발견된 F-1 (정렬 칩↔헤더 불일치) / F-2 (VocTypeBadge 누락) / F-3 (필터 토글 위치 + placeholder) 3건은 PR #175 묶음 + PR #178 minor cleanup 으로 머지 완료. 상세 history는 `progress-archive.md` Wave 1.6 §"Phase C — β batch" 절 참조.

> R5 (bundle 외전 ID 금지) 채택 이전 패턴 — 향후 동급 follow-up은 `followup-bucket.md` 의 `FU-NNN` 으로 부여한다.
