...
3. **폐쇄망 적용 시점에 미러/우회 경로 모두 부재** → 사후 빌드 차단 가능.
   - **현재 전제 (2026-05-01~)**: Wave 0~5는 **개방망 표준 npm registry**로 진행. 폐쇄망 빌드는 동일 lockfile로 사후 재현.
   - **사전 보강**: Wave 0-10 산출물(`phase-8-oss-vendoring.md`)에 §7.2 우회 4경로(미러 / 오프라인 캐시 / Verdaccio / git tarball·`npm pack`) 우선순위로 정리, 각 경로별 절차/한계 명문화.
   - **개방망 진행 중 강제 규칙**: runtime fetch·CDN URL·telemetry 산출물 zero, 폰트/아이콘/MSW worker self-host, `package-lock.json` 항상 commit (폐쇄망 재현 가능성 보존).
   - **롤백**: 폐쇄망 적용 시점에서 우회 경로 4개 모두 실패하면 §7.3 라이브러리별 raw 도입으로 케이스별 대응. Phase 8 자체는 개방망에서 완결.
...
| ID   | 항목                                                                        | 산출 파일                                                                                                                                                                                                                                                                                                 |
| ---- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0-0  | **레지스트리 환경 기록** (Wave 0 첫 commit, 개방망 가정)                    | `npm config get registry` 결과 + `npm ci` 성공 캡처를 `docs/specs/plans/phase-8-mirror-check.md`에 기록. 폐쇄망 사후 재현 4경로(§7.2: 미러 / 오프라인 캐시 / Verdaccio / git tarball·`npm pack`) 우선순위와 사내 신청 플로우도 같은 문서에 정리. **사내 미러 점검은 Phase 8 종료 후 별도 phase에서 수행** |
...
| `docs/specs/archive/plans/phase-8-contract-voc.md` | 본 계약 PR 메모 (의도·스펙 매핑·후속 영향) — Wave 1 머지로 archive                                                                                                 |
...
> **Wave 0 implementation note (2026-05-01)**: 메이저 버전은 설치 시점 stable로 채택 — `zod@^4`, `sonner@^2`, `tailwind-merge@^3`, `lucide-react@^1.14`, `@tanstack/react-query@^5`, `msw@^2`. 위 표의 `^3 / ^1 / ^2 / ^0.4xx` 등은 계획 시점 표기로, 실제 lockfile이 정본.
...
| 폐쇄망 사후 재현 시 미러/우회 경로 부재                              | §7.2 — 우회 4경로(미러 / 오프라인 캐시 / Verdaccio / git tarball·`npm pack`) 우선순위 + §7.3 라이브러리별 raw 도입. Phase 8 진행 자체는 개방망 기준 (2026-05-01 갱신) |

---

## 7. OSS 도입 가이드 — 정본은 별도 문서

> 정본: **`docs/specs/plans/phase-8-oss-vendoring.md`** (Wave 0-10 산출물). 본 절은 포인터로만 유지하며, §7.1~§7.4 본문은 정본 측에서 갱신한다.
