...
> 최종 업데이트: 2026-05-10 (Wave 2 Phase E 머지 완료 · ADR 0006/0007 PR #310 + Wave 3 구현 PR #311 OPEN)
> 현재 위치: **ADR 0006 Accepted + 0007 Proposed (PR #310 OPEN) · Wave 3 Phase A 구현 (PR #311 OPEN, base=`docs/adr-custom-date-range`) · 잔여 = #310 머지 → #311 머지 (또는 rebase) → 마이그 023 운영 적용 → 통합 검증**
...
| **2 (Dashboard)** | [`wave-2-dashboard.md`](./wave-2-dashboard.md) + [`wave-2-phase-c.md`](./wave-2-phase-c.md) + ADR [`0006`](../../adr/0006-custom-date-range-default.md) (Accepted) / [`0007`](../../adr/0007-custom-date-range-timezone.md) (Proposed) | ✅ **Phase D / B / PageFrame / 사이드바 / 스테일 정리 / Phase C BE+FE / vite proxy / Phase E** 모두 머지 (PR #295/#297/#298/#301/#302/#305/#306/#304/#308/#309, 2026-05-10). 🟡 **FU-026 ADR 0006 Accepted + 0007 Proposed (PR #310 OPEN)** + **Wave 3 Phase A 구현 (PR #311 OPEN, base=`docs/adr-custom-date-range`, 마이그 023 + zod superRefine + BE service 415/500/400 + FE CustomDateRangePicker + DateRangeSection 추출 + MSW admin 415 분기 + BE 593 / FE 661 pass)** — 사용자 #310 → #311 순차 머지 → 마이그 023 운영 적용 (rogue row 진단 SQL 선행) → Dialog 'custom' 저장 round-trip 검증. 🟡 **Phase E (DashboardSettingsDialog) 코드 완료 · PR #308 OPEN · 사용자 머지 대기** (2026-05-10) — 커밋 1 base (Sheet 우측 슬라이드인 + 위젯 8 토글 + 기본 날짜 라디오 + 히트맵 X축 + Save/Reset/Cancel + close-while-saving guard, `DashboardShell` `hiddenWidgetIds` 가 children + `layouts[bp]` lockstep 필터 → RGL key invariant. 적대적 리뷰 P0×2 + P1×2 close — RGL key alignment, close race, hiddenWidgetIds memoize, draft-driven custom radio). 커밋 2 Item 1 + Item 2 — Item 1 (admin-default scope toggle): BE `GET /api/dashboard/settings?scope=admin` (admin only, raw row, 403 `FORBIDDEN_ADMIN_SCOPE`) + FE `useDashboardSettings(scope)` / `useUpdateDashboardSettings(scope)` queryKey scoped, mutation success 양 scope invalidate, Dialog `저장 대상` 토글 admin 만 표시 / scope 전환 시 hooks re-key. Item 2 (GlobalTabs reorder): 신규 `GlobalTabsEditor.tsx` native HTML5 DnD + 행별 visibility 토글 + systemId add/remove + empty state, `scope === 'admin'` 일 때만 표시. **Item 3 (`'custom'` 날짜 picker) 의도적 미포함** — 스키마 `custom_start/end_date` 컬럼 부재 → 마이그 필요 → CLAUDE.md irreversibility 게이트 → `FU-026` 등록. 검증: BE typecheck 0 / 587 (+3) · FE typecheck 0 / 661 (+13: base 9 + admin 4) · lint 0 / token lint 0. 오케스트레이션: 사용자 직접 지시 "codex 사용 금지" — 적대적 결정/리뷰 모두 `general-purpose` 에이전트로 대체. |
...
~~FSD Migration~~ → ~~`/voc 완성` PR #242~~ → ~~Wave 4 PR #245~~ → ~~Wave 3 Phase A (4 PR + hotfix)~~ → ~~Wave 3 Phase B+C (PR #262/#263)~~ → ~~Phase D+E 병렬 (PR #269/#270)~~ → ~~Phase F 종합 검증 (PR #271)~~ → ~~FU P1 batch close (PR #273)~~ → ~~Wave 5 plan 작성~~ → ~~Wave 5 Phase A (notifications + comments BE, PR #276)~~ → ~~Wave 5 Phase B PR-2 (PR #278)~~ → ~~P2 BE batch (FU-007/010/015/018, PR #279/#280/#281/#282)~~ → ~~FU-021 / FU-022 P1 batch (PR #284/#285)~~ → ~~FU-023/024/025 P2 후속 (PR #287/#288/#289)~~ → ~~Wave 2 Phase D RGL Shell (PR #295)~~ → ~~Wave 2 Phase B KPI (PR #297 BE / PR #298 FE)~~ → ~~dashboard MSW handler + BodyScrollLayout 8 페이지 마이그 (미커밋)~~ → ~~PageFrame 프리미티브 도입 + VOC 마이그 + AppShell main invariant + PageLayout 삭제 (PR #301)~~ → ~~사이드바 admin 메뉴 누락 픽스 (PR #302)~~ → ~~스테일 브랜치 12건 일괄 정리 (PR #291–#294 close + 원격/로컬 stale 전부 삭제)~~ → ~~vite proxy 포트 정합 (PR #304)~~ → ~~Wave 2 Phase C BE (PR #305)~~ → ~~Wave 2 Phase C FE (PR #306)~~ → ~~Phase E base + admin scope (PR #308/#309)~~ → **FU-026 ADR 0006 Accepted + 0007 Proposed (PR #310 OPEN)** → **Wave 3 Phase A 구현 (PR #311 OPEN, base=#310 브랜치)** → 사용자 #310 머지 → #311 머지 (또는 rebase) → 마이그 023 운영 적용 → 통합 검증 → 운영/배포 phase. ADR 0007 (timezone) 잠금은 다중 timezone 운영 진입 전 별 세션.
