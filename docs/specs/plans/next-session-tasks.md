...
> **정본**: `phase-8.md §Wave 1.5` (2026-05-01 promotion 완료, 인터뷰 9 결정 + 1.5-A/1.5-B PR 스펙).
> **상태**: 인터뷰 종료, 설계/구현계획 사용자 승인 대기. 진행 순서는 phase-8.md §1.5 "구현 진행 순서" 참조.
> **OSS 인벤토리**: `phase-8-oss-vendoring.md §3-bis` (Toast UI lazy-load + 폐쇄망 사후 재현 §2 우회).

### Wave 1 follow-up PRs (Wave 1.5 보강 PR과 병렬 가능)

| ID                              | 항목                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 상태                                              |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| follow-up A (시각 동등화 → e2e) | **autopilot 4단계 완료, PR #125 사용자 검수 대기** (branch `feat/wave1-followup-a-visual-parity`, 2026-05-02). <br>**Stage 1**: `scripts/visual-diff/` Playwright 듀얼-렌더 harness + 12 components data-pcomp markers + MSW `/api/auth/*` durable handlers. <br>**Stage 2**: 13 token-only fixes — visual-diff HIGH 26→9 (잔여 9건은 design `stage2-design.md §2.C` SKIP — table a11y / Radix 내부 / 의도된 summary+nav 레이아웃). <br>**Stage 3**: 24 PNG 캡처 + `voc-visual-screenshots.md` side-by-side 인덱스. <br>**Stage 4**: `frontend/e2e/voc-happy-path.spec.ts` (Playwright 1.8s, login→list→create→drawer→history). <br>**테스트**: 189 vitest pass + 1 e2e pass / lint:tokens clean / typecheck clean. <br>**리뷰**: architect/code-reviewer/security-reviewer/designer/test-engineer 전원 stage별 병렬, self-review 없이 통과. <br>**부수 발견 + 수정**: MSW POST /api/vocs `sequence_no` 누락 (등록 모달 silent 실패) + auth handlers 부재. <br>**검수 포인트**: ① `voc-visual-screenshots.md` 사람 눈 갭 ② 9건 SKIP HIGH ship-OK 여부 ③ overlay (modal/dropdown/drawer) React-side data-pcomp markers 추가 여부 (deferred). | ⏳ PR #125 검수 대기 (2026-05-02 autopilot 완료)  |
...
| F9  | uidesign.md §14 컴포넌트들을 React로 구현 (역할별: AdminTopbar, RolePill 4종, NoticeBadge 3종 등)                     |
...
- **Phase F** ✅ (2026-04-26) — D18~D21 (Dev role / `?mode=admin` 진입점 / uidesign §14 신규 / `assertCanManageVoc`).
