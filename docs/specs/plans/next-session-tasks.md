...
| ID                              | 항목                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 상태                                              |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| follow-up A (시각 동등화 → e2e) | **4단계 순서로 진행** — Wave 1.5-β 머지 후 `/voc`가 prototype과 시각적으로 충분히 유사하지 않다는 사용자 판단(2026-05-02). 픽셀 % 비교는 신뢰 불가 → computed style diff + 사람 검수로 컴포넌트 자체를 끌어올리는 방향. <br>**1) `scripts/visual-diff.ts`** — Playwright 기반, prototype(`prototype/index.html`) vs `/voc` 같은 viewport(1280)에서 띄워 컴포넌트별 셀렉터 매핑 → `getComputedStyle()` 떠서 시각 직결 properties(font / color / spacing / border / shadow / display·flex·grid)만 diff → `voc-visual-diff-report.md` 액션 리스트 출력. <br>**2) 1차 컴포넌트 수정** — 리포트 항목별로 토큰 우선, 그 다음 컴포넌트 클래스. raw hex/px 추가 금지 (uidesign §10·12 토큰만). <br>**3) Playwright 컴포넌트·레이아웃 스크린샷 비교** — `VocTopbar`/`VocStatusFilters`/`VocAdvancedFilters`/`VocSortChips`/`VocTable` row/`VocPaginationBar`/`NotificationBell`/`VocCreateModal` + 전체 페이지(1280·1440) PNG 좌우 저장 → 잔여 갭 보고 컴포넌트 재수정 (2~3회 반복). <br>**4) 사용자 최종 검수** → 픽스 반영 → 그 후 `frontend/e2e/voc-happy-path.spec.ts` 작성 (순수 기능 e2e: 로그인 → 리스트 → 생성 모달 → 등록 → 상세 → 히스토리. **시각 비교 로직 X** — 컴포넌트 정본은 1~3에서 확정되므로 e2e는 기능 회귀만). <br>**원칙**: 정본은 컴포넌트 라이브러리 → 이후 페이지(Wave 2 대시보드 등)는 prototype 1:1 매칭 대신 컴포넌트 재사용 우선. 따라서 시각 회귀 가드는 **공통 primitive 컴포넌트 레벨**에만 (Wave 2 시작 시 별도 도입). | ⏳ Wave 1.5-β 머지 후 (2026-05-02 재계획)         |
