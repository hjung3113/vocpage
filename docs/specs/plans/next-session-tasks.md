...
| Step  | 내용                                                                          | 상태 |
| ----- | ----------------------------------------------------------------------------- | ---- |
| **1** | 토큰 보강 — `--font-code`/`--font-ui` 신설, `#888888` → 토큰, `rgba()` → 토큰 | ✅   |
| **2** | AppShell + Sidebar 골격 — `components/layout/` 신규, RootLayout 재구성        | ✅   |
| **3** | VOC 목록 — 담당자 필터, 컬럼 정렬, 필터 탭 순서(드랍↔완료)                    | ✅   |
| **4** | VocDrawer 탭 재구성 — 서브태스크/내부노트 탭 추가                             | ✅   |
| **5** | Sidebar 동적 시스템 메뉴 + SidebarUser                                        | ✅   |
| **6** | 대시보드 재구성 — KPI 8종, GlobalTabs, StatusDonut                            | ✅   |
| **7** | Admin 공지/FAQ 관리 탭 CRUD + NoticePopup dismiss                             | ✅   |
| **8** | 알림 패널 UI + UrgentBadge + markAllAsRead                                    | ✅   |
...
| ID    | 내용                                                                           | 상태 |
| ----- | ------------------------------------------------------------------------------ | ---- |
| R7-8  | `autoTag.ts` 단건 INSERT 루프 → VALUES 배치 INSERT (pg-mem 호환)               | ✅   |
| R7-9  | `autoTag.ts` tag_rules 조회 5분 TTL 인메모리 캐싱                              | ✅   |
| R7-10 | `vocs.ts` 1015줄 → `routes/subtasks.ts`, `routes/payload.ts` 분리              | ✅   |
| R7-11 | `VocPage.tsx` React Query `staleTime` 명시적 설정 — React Query 미사용으로 N/A | skip |
