...
> 최종 업데이트: 2026-05-10 (Wave 2 Phase C BE PR #305 + FE PR #306 머지 + vite proxy 정합 PR #304)
> 현재 위치: **Phase C 완료 · Phase B/C/D 모두 머지 · 잔여 = Phase E (DashboardSettingsDialog) → 운영/배포**
...
| **2 (Dashboard)** | [`wave-2-dashboard.md`](./wave-2-dashboard.md) + [`wave-2-phase-c.md`](./wave-2-phase-c.md) (Phase B/C/D 완료, E 후속)                                                  | ✅ **Phase D / B / PageFrame / 사이드바 / 스테일 정리** (PR #295/#297/#298/#301/#302, 2026-05-10). ✅ **Phase C BE 머지 완료** (PR #305, 2026-05-10) — 7 endpoint (`distribution / priority-status-matrix / heatmap / weekly-trend / processing-speed / assignee-stats / aging-vocs`) + 7 zod 컨트랙트 + `dashboard-phase-c.{route,service,repo}.ts` + 권한 매트릭스 (admin/manager/dev 200, user 403). codex:rescue P0×2 (KST 캘린더 캐스팅 SQL · `(status_changed_at AT TIME ZONE 'Asia/Seoul')::date - (created_at ...)::date` 3 site) + P1×1 (`range=custom` validation refine) 픽스 동봉. BE 518 → 584 (+66). ✅ **Phase C FE 머지 완료** (PR #306, 2026-05-10) — 7 위젯 + `GridTable.tsx` 공용 (heatmap/matrix/assignee 강도 그리드) + `buildVocUrl.ts` (5 위젯 click-through nav · globalFilter 컨텍스트 보존) + recharts@^3.8.1 + MSW 7 핸들러. codex:rescue P0×3 (matrix dead code → `dist-matrix` 슬롯 vertical stack · click-through 5 위젯 wiring · SLA 토큰 `chart-*` → `status-*`) + P1×4 (heatmap 빈 카피 / 3-level breadcrumb / AgingVocs dim toggle / AssigneeStats highlight) 픽스 동봉. FE 603 → 648 (+45). **Spec amendment** — `dashboard.md §5` v3.1: `inProgress` 의미 "주말 스냅샷" → "해당 주에 검토중/처리중 진입" (status_history 테이블 부재). ✅ **vite proxy 포트 정합** (PR #304) — BE 기본 PORT 3000 → 3001 (vite proxy 일치). |
...
~~FSD Migration~~ → ~~`/voc 완성` PR #242~~ → ~~Wave 4 PR #245~~ → ~~Wave 3 Phase A (4 PR + hotfix)~~ → ~~Wave 3 Phase B+C (PR #262/#263)~~ → ~~Phase D+E 병렬 (PR #269/#270)~~ → ~~Phase F 종합 검증 (PR #271)~~ → ~~FU P1 batch close (PR #273)~~ → ~~Wave 5 plan 작성~~ → ~~Wave 5 Phase A (notifications + comments BE, PR #276)~~ → ~~Wave 5 Phase B PR-2 (PR #278)~~ → ~~P2 BE batch (FU-007/010/015/018, PR #279/#280/#281/#282)~~ → ~~FU-021 / FU-022 P1 batch (PR #284/#285)~~ → ~~FU-023/024/025 P2 후속 (PR #287/#288/#289)~~ → ~~Wave 2 Phase D RGL Shell (PR #295)~~ → ~~Wave 2 Phase B KPI (PR #297 BE / PR #298 FE)~~ → ~~dashboard MSW handler + BodyScrollLayout 8 페이지 마이그 (미커밋)~~ → ~~PageFrame 프리미티브 도입 + VOC 마이그 + AppShell main invariant + PageLayout 삭제 (PR #301)~~ → ~~사이드바 admin 메뉴 누락 픽스 (PR #302)~~ → ~~스테일 브랜치 12건 일괄 정리 (PR #291–#294 close + 원격/로컬 stale 전부 삭제)~~ → ~~vite proxy 포트 정합 (PR #304)~~ → ~~Wave 2 Phase C BE (PR #305)~~ → ~~Wave 2 Phase C FE (PR #306)~~ → **Phase E (DashboardSettingsDialog)** → 운영/배포 phase.
