...
| FU-017 | Page header drift audit (5 pages, 2026-05-09)        | Page Header / Action size system 시행 — `<PageLayout>` + `<PageHeader>` + `--ui-h-*` 토큰 + 5 page 마이그         | Open   | P1. `uidesign.md §5 Page Header — Slot contract` 신규 명세 시행. 단일 PR 로 묶어 머지 (브랜치 `docs/page-layout-system`): spec → tokens+component → Button/Input 토큰화 → VOC → Notice → FAQ → Tag Master → Trash. `benchmark/` 폐기 (2026-05-09 사용자 결정). |
