...
| FU-013 | Wave 3 Phase C (PR #263 codex P1, 2026-05-09)        | `vocs.parent_id` ON DELETE SET NULL 마이그 — D13 정합 (003 미적용)                                                | Open   | P1. ADR 0005 §6 case 6 (parent hard-delete 후 sub-task `parent_id=NULL` 복원) 정합. 별 마이그 + rollback.                                                                                                                                                             |
| FU-014 | Wave 3 Phase B (PR #262 codex P1, 2026-05-09)        | `tags(name, kind)` UNIQUE 제약 — 현재 `slug` UNIQUE 우연 의존                                                     | Open   | P1. 동일 name 다른 kind 정상 케이스인데 slug 충돌로 실패 가능. 별 마이그 + 회귀 테스트.                                                                                                                                                                               |
...
| FU-017 | Page header drift audit (5 pages, 2026-05-09)        | Page Header / Action size system 시행 — `<PageLayout>` + `<PageHeader>` + `--ui-h-*` 토큰 + 5 page 마이그         | Open   | P1. `uidesign.md §5 Page Header — Slot contract` 신규 명세 시행. 7 PR: (1) tokens + `<PageLayout>`/`<PageHeader>`, (2) Button/Input 토큰화 (h-9/10/11 → `--ui-h-*`), (3) VOC, (4) Notice, (5) FAQ, (6) Tag Master, (7) Trash. baseline 갱신은 페이지별 PR 에서 1개씩. |
