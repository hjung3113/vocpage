...
> **v3.1 (2026-05-10) 정정:** `진행중(검토+처리)` 선의 계산 기준을 정정함.
> `enteredInProgress` 의미 — status_history 테이블 부재로 진정한 스냅샷 불가. 진입 카운트로 정정.
> 원 spec "주말 스냅샷 기준 검토중+처리중 상태 VOC 수" → "해당 주에 검토중 또는 처리중으로 전환된 VOC 수"로 변경.
...
| 진행중(검토+처리) | 해당 주에 검토중 또는 처리중으로 전환된 VOC 수 (v3.1 정정: 주말 스냅샷 → 진입 카운트) | `status_changed_at` 기준 + `status IN (검토중,처리중)` | `var(--chart-sky)`     |
...
> `진행중` 선은 스냅샷이 아닌 진입 카운트이므로, 같은 주에 검토중→처리중으로 재전환된 VOC가 중복 카운트될 수 있음. 이는 status_history 테이블 없이는 해결 불가 — 허용된 근사값.
...
{
  weeks: string[],                  // ['W1', ..., 'W12']
  weekStarts: string[],             // ISO Monday dates for each week
  series: {
    new:              number[],     // 해당 주 신규 접수 (created_at)
    enteredInProgress: number[],    // 해당 주 검토중+처리중으로 전환된 VOC 수
                                    // (v3.1 정정: 'inProgress' → 'enteredInProgress')
    done:             number[]      // 해당 주 완료+드랍 전환 (status_changed_at)
  }
}
```
