...
| VOC Detail Drawer     | row 클릭 (`onclick="openDrawer(id)"`) | `prototype.html:L874–L891` (`#drawerOverlay` + `#drawer`) | `features/voc/components/VocReviewDrawer.tsx` (+ `VocReviewSections.tsx`) |
...
└─ div.drawer-body#drawerBody    ← [AMENDED C-13 2026-05-04] prototype은 평평 섹션(.d-section sibling)만 사용, 탭 UI 없음. 위 ".d-overview/..." 명칭은 production 초기 분해 기록이며 탭 폐기 후 무효
```
...
- ~~탭 전환 → 활성 `.d-tab-btn.active` + 해당 `.d-section` 표시~~ **[AMENDED C-13 2026-05-04]** prototype에 탭 UI 없음 — `.d-section` 들이 평평하게 동시 노출. 이 항목은 무효.

인터랙션 (drawer.js, drawer-core.js, drawer-advanced.js, drawer-comments.js, internal-notes.js):

- `openDrawer(vocId)` → VOC_MAP 조회 → 평평 섹션 빌드(코멘트/첨부/변경이력 + ε/η 추가 섹션) → role gate (drawer.js:L3 `applyDrawerUserReadonly`). **[AMENDED C-13 2026-05-04]** 원문은 "탭 빌드(개요/댓글/내부노트/어드밴스드)" 였으나 prototype 재확인 결과 탭 없음 — production 도 VocReviewSections 평평 구조로 정렬.
...
Production 대응: `frontend/src/features/voc/components/VocReviewDrawer.tsx` + `VocReviewSections.tsx`.
...
| VocReviewSections        | 평평 섹션 컨테이너 (탭 폐기)            | voc, role                        | section-visible                 | comments/attachments/history (+ε/η 슬롯) | drawer-core.js + drawer-comments.js + internal-notes.js + drawer-advanced.js | `features/voc/components/VocReviewSections.tsx`                          | **rebuild** (Wave 1.6 C-13 — 2026-05-04 prototype 재확인 결과 prototype에 탭 UI 없음 확인. 코멘트/첨부/변경이력 3개 섹션을 평평하게 노출, 후속 batch (ε/η)에서 children/composition으로 추가 섹션 끼움)                                                                                                           |
| VocCommentList           | 댓글 목록 + 편집 메뉴                   | comments, currentUserId          | menu open                       | own/other                                | `drawer-comments.js`                                                         | (VocReviewSections 내)                                                   | **rebuild**                                                                                                                                                                                                                                                                                                       |
| VocInternalNotes         | 내부 노트                               | vocId, role                      | role-gated                      | —                                        | `internal-notes.js`                                                          | (VocReviewSections 내)                                                   | **rebuild**                                                                                                                                                                                                                                                                                                       |
| VocSubTaskList           | 서브태스크 (drawer)                     | items, onToggle                  | hover/done                      | —                                        | `drawer-core.js:L22`                                                         | (VocReviewSections 내)                                                   | **rebuild**                                                                                                                                                                                                                                                                                                       |
...
> 4. VocCommentList → VocInternalNotes → VocSubTaskList → VocReviewSections → VocReviewDrawer (탭 폐기 — Wave 1.6 C-13)
...
| VocReviewSections × 3 | 1.0               | 평평 섹션 (코멘트/첨부/변경이력) — 탭 폐기 (C-13) |
