...
- **발생 조건:**
  ...
  - 내 VOC의 상태 변경
  - 나에게 담당자 배정
- **수신자 (Wave 5 Phase A 명문화):** 댓글 / 상태 변경은 **VOC 작성자 + 담당자 둘 모두** (`§3` line 52 와 정합). 본인 행위는 수신자에서 제외. 담당자 배정은 신규 담당자 단일.
- **UI:** 벨 아이콘 + 미읽음 배지(최대 표시 99+). 클릭 시 알림 패널 열림 및 읽음 처리. **Urgent VOC 관련 알림(배정·상태변경)은 벨 아이콘 옆에 빨간 느낌표(🔴!) 배지 추가** — 일반 알림 배지와 시각 구분. 느낌표 배지는 해당 Urgent 알림을 읽으면 해제. **Urgent 표시 기준 = 표시 시점 VOC priority** (Urgent 해제 시 🔴! 자동 해제, 응답 DTO 의 `voc.priority` join 으로 계산 — Wave 5 W5-D2).
- **보관:** 최근 50건 표시. 51번째부터 오래된 순 삭제. 30일 초과 알림은 자동 삭제 (Wave 5 W5-D4: read 무관, 목록 조회 시점 lazy trim — cron / 마이그 회피). 두 기준 중 먼저 도달한 쪽 적용.
- **중복 억제:** 동일 VOC에서 같은 유형의 알림은 5분 내 디바운스 (앱 레이어 + DB 인덱스 조회, 단일 인스턴스 전제 — Wave 5 W5-D3).
