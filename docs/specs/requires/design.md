...
- Each status pill includes a Lucide icon (11px) left of label:
...
- Count badge on nav items — two variants:
  - **Accent** (total VOC count): `background: var(--brand); color: #fff`
...
Slides in from the right. Width: **528px**. Background: `var(--bg-panel)`.
...
- Issue code: `font-family: var(--font-code); font-size: 11px; font-weight: 600; color: var(--accent); letter-spacing: 0.02em`
...
- Container: `display: grid; grid-template-columns: 1fr 1fr; gap: 12px; padding: 16px 24px; border-bottom: 1px solid var(--border-subtle)`
- Field label: `font-size: 10.5px; font-weight: 600; color: var(--text-quaternary); text-transform: uppercase; letter-spacing: 0.07em`
- Field value: `font-size: 13px; color: var(--text-secondary); display: flex; align-items: center; gap: 6px`
- Editable fields (상태, 우선순위 for manager/admin): `<select>` with `border: 1px solid var(--border-subtle); border-radius: 6px; padding: 5px 9px; background: var(--bg-elevated)`

**Tags Row**:

```
태그 [chip] [chip] [+ 태그 추가]       [⚡ 자동 태깅]
```

- Layout: `display: flex; align-items: center; justify-content: space-between; padding: 12px 24px; border-bottom: 1px solid var(--border-subtle)`
- 자동 태깅 button: `font-size: 11px; color: var(--accent)` — no border, no background

### Initials Avatar

Used for 담당자, 작성자 in the drawer and for user identity in the sidebar.
...
- Background color: deterministic hash of the name string, cycling through chart palette tokens (`tokens.chartBlue`, `chartSky`, `chartEmerald`, `chartAmber`, `chartRed`, `chartTeal`, `chartIndigo`, `brand`)
- First character of display name as the avatar text
- Never use brand blue as the only option — the palette cycle ensures differentiation
